...
def classify():
    ...
    Xt = np.array(xt)
    Yt = np.array(yt)
    model = svm.SVC(kernel='poly', degree=8, coef0 =1,C=3)
    model.fit(X,Y)
    py = model.predict(Xt)
    ...
    loo = LeaveOneOut()
    for c in range(50):
        model = svm.SVC(kernel='poly', degree=8, coef0 =1,C=(c+1))
        error = 0
        for train_index, test_index in loo.split(X):
            ...
            error += (1 - np.sum(py == Y_test))
        print("C: ", (c+1))
        print("Etest: ", float(error)/test_index)
    """
    ...
    plt.title("Decision boundary for $C = 3$")
    #plt.grid()
